...
public interface AudioMixerByteIFace {
     ...
     *
     * @param vals           is the 2D array with samples. vals.length is number of mixed values.
     *                       And we take values from vals[][index] to vals[][index + sampleSize] and mix them
    ...
    /**
     * Mixes vals.length values to 1 sample. vals.length == multFactors.length,
     * multFactors[].length == number of channels in output audioFormat
     *
     * @param vals        is the 2D array with samples, where vals[i].length == sampleSize and
     *                    vals.length == number of samples to mix
